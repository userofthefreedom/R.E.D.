...
function createParcelPolygon(lon: number, lat: number) {
  return new Polygon([
    [
      ...
      fromLonLat([lon + 0.001, lat + 0.00105]),
      fromLonLat([lon + 0.00145, lat - 0.0008]),
      fromLonLat([lon - 0.00105, lat - 0.0011]),
      fromLonLat([lon - 0.00135, lat + 0.00075]),
    ],
  ]);
}
  ...
  <div class="map-stack">
    <div class="map-mock actual-map" aria-label="선택 필지 실제 지도">
      ...
      <div class="map-tools">
        ...
        <button class="tool-square" title="선택 필지로 이동" @click="recenter">
          <LocateFixed />
        </button>
      </div>
      <div class="map-watermark">
        <MapPin /> {{ mapProviderLabel }} 지도 · 클릭 위치 선택
      </div>
    </div>
